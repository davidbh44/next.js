...
Generating source maps and using them in Node.js consumes extra memory during the build process.
...
you can try disabling source maps in that phase by adding `staticPageGenerationSourcemaps: false` to your Next.js configuration.
Keep in mind that disabling source maps during prerender will negatively impact
debugging of errors during prerender since the stack trace will be unreadable.
